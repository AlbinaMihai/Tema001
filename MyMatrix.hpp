...
private:
	...
	void GetCofactor(T *&A, T *&Temp, int p, int q, int n)  const
	{
		int i = 0, j = 0;
		int N = this->Rows;
	...
	float GetDeterminant(T *&Aux, int n) const
	{
		float D = 0;
...
public:
	...
	const int GetRowsNr() { return Rows; }
    const int GetColomnsNr() { return Colomns; }
	const T GetElement(int L, int C) { return A[L*Colomns + C]; }
	...
	void Inverse()
	{
		...
		T *Adj = Adjoint();
		Inv = (T*)calloc(Rows*Rows, sizeof(T));
